...
public:
	...
	static constexpr long MAX_BITS = FFTReal_get_next_pow2(max_length);
...
private:
   ...
   static constexpr int TRIGO_BD_LIMIT	= 12;
	...
	long		_length;
	int		_nbr_bits;
	// std::vector <long> _br_lut;
	// std::vector <DataType> _trigo_lut;
	// mutable std::vector <DataType> _buffer;
    // mutable std::vector <OscType> _trigo_osc;


	long _br_lut[1L << MAX_BITS];
	DataType _trigo_lut[MAX_BITS > 3 ? ((1L << (MAX_BITS - 1)) - 4) : 1];
	mutable DataType _buffer[max_length];
    mutable OscType _trigo_osc[MAX_BITS > TRIGO_BD_LIMIT ? MAX_BITS - TRIGO_BD_LIMIT : 1];
