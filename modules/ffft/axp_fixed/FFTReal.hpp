...
template <class DT, long max_length>
FFTReal <DT, max_length>::FFTReal (long length)
:	_length (length)
,	_nbr_bits (FFTReal_get_next_pow2 (length))
//,	_br_lut(1L << _nbr_bits)
//,	_trigo_lut (_nbr_bits > 3 ? ((1L << (_nbr_bits - 1)) - 4) : 0)
//,	_buffer (length)
//,	_trigo_osc (_nbr_bits > TRIGO_BD_LIMIT ? _nbr_bits - TRIGO_BD_LIMIT : 0)
{
	assert (FFTReal_is_pow2 (length));
	assert (_nbr_bits <= MAX_BIT_DEPTH);
	assert(_nbr_bits <= MAX_BITS);
...
template <class DT, long max_length>
void FFTReal<DT, max_length>::set_length(long length)
{
    assert(FFTReal_is_pow2(length));
    assert(_nbr_bits <= MAX_BIT_DEPTH);
...
template <class DT, long max_length>
long	FFTReal <DT, max_length>::get_length () const
{
	return _length;
}
...
template <class DT, long max_length>
void	FFTReal <DT, max_length>::do_fft (DataType f [], const DataType x []) const
{
	assert (f != nullptr);
	assert (f != use_buffer ());
	assert (x != nullptr);
...
template <class DT, long max_length>
void	FFTReal <DT, max_length>::do_ifft (const DataType f [], DataType x []) const
{
	assert (f != nullptr);
	assert (f != use_buffer ());
	assert (x != nullptr);
...
template <class DT, long max_length>
void	FFTReal <DT, max_length>::rescale (DataType x []) const
{
...
template <class DT, long max_length>
typename FFTReal <DT, max_length>::DataType *	FFTReal <DT, max_length>::use_buffer () const
{
	return _buffer;
}
...
template <class DT, long max_length>
void	FFTReal <DT, max_length>::init_br_lut ()
{
	const long		length = 1L << _nbr_bits;
	//_br_lut.resize (length);
    assert(length <= _br_lut.size());
...
template <class DT, long max_length>
void	FFTReal <DT, max_length>::init_trigo_lut ()
{
	if (_nbr_bits > 3)
	{
		const long		total_len = (1L << (_nbr_bits - 1)) - 4;
		//_trigo_lut.resize (total_len);
        assert(total_len <= _trigo_lut.size());
...
template <class DT, long max_length>
void	FFTReal <DT, max_length>::init_trigo_osc ()
{
	const int		nbr_osc = _nbr_bits - TRIGO_BD_LIMIT;
	if (nbr_osc > 0)
	{
		//_trigo_osc.resize (nbr_osc);
        assert(nbr_osc <= _trigo_osc.size());
...
template <class DT, long max_length>
const long *	FFTReal <DT, max_length>::get_br_ptr () const
{
	return (&_br_lut [0]);
}
...
template <class DT, long max_length>
const typename FFTReal <DT, max_length>::DataType *	FFTReal <DT, max_length>::get_trigo_ptr (int level) const
{
	assert (level >= 3);
...
template <class DT, long max_length>
long	FFTReal <DT, max_length>::get_trigo_level_index (int level) const
{
	assert (level >= 3);
...
template <class DT, long max_length>
void	FFTReal <DT, max_length>::compute_fft_general (DataType f [], const DataType x []) const
{
	assert (f != nullptr);
	assert (f != use_buffer ());
	assert (x != nullptr);
...
template <class DT, long max_length>
void	FFTReal <DT, max_length>::compute_direct_pass_1_2 (DataType df [], const DataType x []) const
{
	assert (df != nullptr);
	assert (x  != nullptr);
	assert (df != x);
...
template <class DT, long max_length>
void	FFTReal <DT, max_length>::compute_direct_pass_3 (DataType df [], const DataType sf []) const
{
	assert (df != nullptr);
	assert (sf != nullptr);
	assert (df != sf);
...
template <class DT, long max_length>
void	FFTReal <DT, max_length>::compute_direct_pass_n (DataType df [], const DataType sf [], int pass) const
{
	assert (df != nullptr);
	assert (sf != nullptr);
	assert (df != sf);
...
template <class DT, long max_length>
void	FFTReal <DT, max_length>::compute_direct_pass_n_lut (DataType df [], const DataType sf [], int pass) const
{
	assert (df != nullptr);
	assert (sf != nullptr);
	assert (df != sf);
...
template <class DT, long max_length>
void	FFTReal <DT, max_length>::compute_direct_pass_n_osc (DataType df [], const DataType sf [], int pass) const
{
	assert (df != nullptr);
	assert (sf != nullptr);
	assert (df != sf);
...
template <class DT, long max_length>
void	FFTReal <DT, max_length>::compute_ifft_general (const DataType f [], DataType x []) const
{
	assert (f != nullptr);
	assert (f != use_buffer ());
	assert (x != nullptr);
...
template <class DT, long max_length>
void	FFTReal <DT, max_length>::compute_inverse_pass_n (DataType df [], const DataType sf [], int pass) const
{
	assert (df != nullptr);
	assert (sf != nullptr);
	assert (df != sf);
...
template <class DT, long max_length>
void	FFTReal <DT, max_length>::compute_inverse_pass_n_lut (DataType df [], const DataType sf [], int pass) const
{
	assert (df != nullptr);
	assert (sf != nullptr);
	assert (df != sf);
...
template <class DT, long max_length>
void	FFTReal <DT, max_length>::compute_inverse_pass_n_osc (DataType df [], const DataType sf [], int pass) const
{
	assert (df != nullptr);
	assert (sf != nullptr);
	assert (df != sf);
...
template <class DT, long max_length>
void	FFTReal <DT, max_length>::compute_inverse_pass_3 (DataType df [], const DataType sf []) const
{
	assert (df != nullptr);
	assert (sf != nullptr);
	assert (df != sf);
...
template <class DT, long max_length>
void	FFTReal <DT, max_length>::compute_inverse_pass_1_2 (DataType x [], const DataType sf []) const
{
	assert (x  != nullptr);
	assert (sf != nullptr);
	assert (x  != sf);
